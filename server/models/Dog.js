...
class Dog {
  constructor(dog) {
    ...
    this.timestamp = (
      dog.timestamp instanceof Date ? dog.timestamp : new Date(dog.timestamp)
    )
      .toISOString()
      .replace(/T/, " ")
      .replace(/\..+/, "");
  ...
  static async show(id) {
    const response = await db.query("SELECT * FROM dogs WHERE dog_id = $1;", [
      id,
    ]);
  ...
  static async create(data) {
    const {
      ...
      size,
      breed,
      young_children_compatibility,
      small_animal_compatibility,
      activity_levels,
      living_space_size,
      garden,
      allergenic,
    ...
    const existingDog = await db.query("SELECT * FROM dogs WHERE dog_id = $1", [
      dog_id,
    ]);
  ...
  async update(data) {
    ...
    const response = await db.query(
      ...
      [
        ...
        this.size,
        this.breed,
        this.young_children_compatibility,
        this.small_animal_compatibility,
        this.activity_levels,
        this.living_space_size,
        this.garden,
        this.allergenic,
        this.other_animals,
        this.fencing,
